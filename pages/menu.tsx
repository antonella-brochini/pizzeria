...
const Menu = () => {
  ...
  const { items, cart } = useSelector((state: any) => ({
    items: state.items,
    cart: state.cart,
  }));

  useEffect(() => {
    // si tu slice guarda items como array directamente:
    if (!items || items.length === 0) {
      dispatch(getItems());
    }

    // si tu slice guarda items como { items: [] } entonces sería:
    // if (!items?.items || items.items.length === 0) dispatch(getItems());
  }, [dispatch]);

  const food = items?.find((group: any) => group.type === current_food);
      ...
      <div className="max-w-screen-xl px-2 mx-auto w-full">
        ...
        <div className="flex justify-between items-center md:space-y-0 space-y-4 md:flex-row flex-col max-w-screen-md mx-auto mb-10">
          ...
          >
            {menuIcons.map((icon, index) => (
              <div
                ...
                `}
              >
                <img src={icon.src} className="w-10 h-10 mx-auto" alt={icon.name} />
          ...
          >
            <Link
              href="/customize"
              className="focus:outline-none transition-all hover:shadow-btn_lg shadow-btn px-4 py-2 font-bold rounded bg-primary text-white font-main text-sm"
            >
              Customize order
            </Link>
          </motion.div>
        ...
        >
     {foodItems.length === 0 && <p>Food Not Available Yet</p>}

{foodItems.map(({ id, item_name, item_image, price }: any) => (
            <MenuItem
              key={id}
              cart={cart}
