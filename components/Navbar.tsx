import { signInWithGoogle, signOut } from "@/redux/slices/authSlice";
    ...
    <header className="absolute top-0 w-full py-4">
      <nav className="flex max-w-screen-xl px-2 items-center mx-auto justify-between">
        ...
        >
          ...
          {!authLoading && !userExists && (
            <Link href="/register">
              <div className="transition-all hover:shadow-btn px-4 py-1 font-bold rounded border md:border-primary md:text-primary font-main uppercase">
                Log in
              </div>
            </Link>
          )}
